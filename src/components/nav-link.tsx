...
const NavLink = component$<Props>(({ title, icon, stroke, slug, isActive, newCount }) => {
  return (
    ...
    >
      ...
      {newCount ? (
        <span class='absolute top-0 -right-2 flex items-center justify-center text-xs text-brand-background text-center font-bold rounded-full bg-brand-tertiary w-4 h-4'>
          {newCount}
        </span>
      ) : null}
      {title}
