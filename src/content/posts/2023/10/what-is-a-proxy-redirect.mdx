...
or, if you're deploying to Vercel or Netlify, they're called <b class='text-brand-primary'>Rewrites</b>.
...
```javascript
<Image src={`${process.env.NEXT_PUBLIC_REWRITE_PREFIX}/static/cta-elephant.avif`} width={1684} height={964} alt="Neon Elephant" />
```
  ...
  return (
    <Html lang="en">
      ...
      <body>
        ...
        <NextScript />
        {isProd ? (
          <Script src={`https://www.googletagmanager.com/gtag/js?id=${process.env.NEXT_PUBLIC_GTM}`} strategy="afterInteractive" />
        ) : null}
      </body>
    </Html>
  );
...
If you get stuck, feel free to find me out on Twitter/X, I might be able to help: [@PaulieScanlon](https://x.com/P121129E).
