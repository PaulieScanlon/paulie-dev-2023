...
In this post, I'll give you a quick guide on getting started with Hugging Face [Inference Endpoints](https://huggingface.co/inference-endpoints/dedicated) and their JavaScript SDK: [@huggingface/inference](https://www.npmjs.com/package/@huggingface/inference). I'll walk you through using the `textGeneration` method with a "dedicated inference endpoint."

To explore the models available for this method, visit: [https://huggingface.co/tasks/text-generation](https://huggingface.co/tasks/text-generation). Please note that not all models listed will work, and some might show an **Error: Forbidden:** message.

During the writing of this post, I tested three models from that page that do work. Each model provides a unique response to the same question. I've categorized them as **good**, **bad**, and **ugly**:
...
Before diving in, make sure to complete the following prerequisites.
